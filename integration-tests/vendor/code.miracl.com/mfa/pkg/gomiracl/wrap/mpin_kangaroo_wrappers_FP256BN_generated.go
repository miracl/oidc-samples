...
// Server2FP256BNKangaroo is a wrapper of bindings.MPIN_FP256BN_SERVER_2.
func Server2FP256BNKangaroo(d int, HID []byte, HTID []byte, y []byte, SS []byte, U []byte, UT []byte, V []byte, Pa []byte) (E []byte, F []byte, err error) {
	...
	ESize := bindings.GTSFP256BN
	EOct := bindings.MakeOctet(ESize)
	defer EOct.Free()

	FSize := bindings.GTSFP256BN
	...
	err = bindings.Server2FP256BN(d, HIDOct, HTIDOct, yOct, SSOct, UOct, UTOct, VOct, EOct, FOct, PaOct)
...
// ServerFP256BNKangaroo is a wrapper of bindings.MPIN_FP256BN_SERVER.
func ServerFP256BNKangaroo(h int, d int, SS []byte, U []byte, UT []byte, V []byte, ID []byte, MESSAGE []byte, t int, Pa []byte) (HID []byte, HTID []byte, y []byte, E []byte, F []byte, err error) {
	...
	err = bindings.ServerFP256BN(h, d, HIDOct, HTIDOct, yOct, SSOct, UOct, UTOct, VOct, EOct, FOct, IDOct, MESSAGEOct, t, PaOct)
