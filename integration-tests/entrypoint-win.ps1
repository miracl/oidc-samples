...
if ($connection.Connected) {
    go test -v -mod=vendor . --client-id $Env:CLIENT_ID --client-secret $Env:CLIENT_SECRET --project-domain $Env:PROJECT_DOMAIN --sample-url $Env:SAMPLE_URL --redirect-url $Env:REDIRECT_URL --sample-name $Env.SAMPLE_NAME --skip-modify-tests $Env:SKIP_MODIFY_TESTS
    exit $LASTEXITCODE
} else {
